...
public class Component2EditPart extends AbstractBorderedShapeEditPart implements PrimaryShapeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof ComponentStereo2EditPart) {
			((ComponentStereo2EditPart) childEditPart).setLabel(getPrimaryShape().getFigureComponentFigure_stereo());
			return true;
		}
	...
	protected EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(ComponentStereo2EditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
