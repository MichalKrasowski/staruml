...
public class ComponentItemSemanticEditPolicy extends UMLBaseItemSemanticEditPolicy {
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (UMLVisualIDRegistry.getVisualID(outgoingLink) == ComponentRequiredEditPart.VISUAL_ID) {
				...
				for (Iterator cit = node.getChildren().iterator(); cit.hasNext();) {
					...
					case InterfaceEditPart.VISUAL_ID:
						for (Iterator it = cnode.getTargetEdges().iterator(); it.hasNext();) {
							...
							if (UMLVisualIDRegistry.getVisualID(incomingLink) == ComponentRequiredEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(incomingLink.getSource().getElement(), null, incomingLink.getTarget().getElement(), false);
								cmd.add(new DestroyReferenceCommand(r));
								cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
								continue;
