...
public class ArtifactEditPart extends ShapeNodeEditPart implements PrimaryShapeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof ArtifactContentsEditPart) {
			IFigure pane = getPrimaryShape().getFigureArtifactFigure_body();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.add(((ArtifactContentsEditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof ArtifactContentsEditPart) {
			IFigure pane = getPrimaryShape().getFigureArtifactFigure_body();
			pane.remove(((ArtifactContentsEditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	public List/*<org.eclipse.gmf.runtime.emf.type.core.IElementType>*/getMARelTypesOnSourceAndTarget(IGraphicalEditPart targetEditPart) {
		...
		if (targetEditPart instanceof org.eclipse.uml2.diagram.component.edit.parts.ArtifactEditPart) {
			types.add(UMLElementTypes.Dependency_4009);
		}
	...
	protected EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(ArtifactStereo2EditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
