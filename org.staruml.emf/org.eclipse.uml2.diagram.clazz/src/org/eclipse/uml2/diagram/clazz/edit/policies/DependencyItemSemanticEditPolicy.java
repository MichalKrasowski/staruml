...
public class DependencyItemSemanticEditPolicy extends UMLBaseItemSemanticEditPolicy {
	...
	public DependencyItemSemanticEditPolicy() {
		super(UMLElementTypes.Dependency_2009);
	}
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (UMLVisualIDRegistry.getVisualID(outgoingLink) == DependencySupplierEditPart.VISUAL_ID) {
			...
			if (UMLVisualIDRegistry.getVisualID(outgoingLink) == DependencyClientEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(outgoingLink.getSource().getElement(), null, outgoingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), outgoingLink));
				continue;
