...
import org.eclipse.uml2.diagram.clazz.edit.policies.Class5CanonicalEditPolicy;
import org.eclipse.uml2.diagram.clazz.edit.policies.Class5ItemSemanticEditPolicy;
 ...
 * @generated
 */
public class Class5EditPart extends AbstractBorderedShapeEditPart implements PrimaryShapeEditPart {
	...
	protected void createDefaultEditPolicies() {
		...
		installEditPolicy(EditPolicyRoles.CANONICAL_ROLE, new Class5CanonicalEditPolicy());
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof ClassName2EditPart) {
			((ClassName2EditPart) childEditPart).setLabel(getPrimaryShape().getFigureClassFigure_NameLabel());
			return true;
		}
		if (childEditPart instanceof ClassStereotype2EditPart) {
			((ClassStereotype2EditPart) childEditPart).setLabel(getPrimaryShape().getFigureClassFigure_StereoLabel());
			return true;
		}
		if (childEditPart instanceof ClassAttributes2EditPart) {
			...
			pane.add(((ClassAttributes2EditPart) childEditPart).getFigure());
			return true;
		}
		if (childEditPart instanceof ClassOperations2EditPart) {
			IFigure pane = getPrimaryShape().getFigureClassFigure_OperationsCompartment();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.add(((ClassOperations2EditPart) childEditPart).getFigure());
			return true;
		}
		if (childEditPart instanceof ClassClasses2EditPart) {
			IFigure pane = getPrimaryShape().getFigureClassFigure_ClassesCompartment();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.add(((ClassClasses2EditPart) childEditPart).getFigure());
			return true;
		}
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof ClassAttributes2EditPart) {
			IFigure pane = getPrimaryShape().getFigureClassFigure_PropertiesCompartment();
			pane.remove(((ClassAttributes2EditPart) childEditPart).getFigure());
			return true;
		}
		if (childEditPart instanceof ClassOperations2EditPart) {
			IFigure pane = getPrimaryShape().getFigureClassFigure_OperationsCompartment();
			pane.remove(((ClassOperations2EditPart) childEditPart).getFigure());
			return true;
		}
		if (childEditPart instanceof ClassClasses2EditPart) {
			IFigure pane = getPrimaryShape().getFigureClassFigure_ClassesCompartment();
			pane.remove(((ClassClasses2EditPart) childEditPart).getFigure());
			return true;
		}
		if (childEditPart instanceof PortEditPart) {
	...
	protected IFigure getContentPaneFor(IGraphicalEditPart editPart) {
		if (editPart instanceof ClassAttributes2EditPart) {
			return getPrimaryShape().getFigureClassFigure_PropertiesCompartment();
		}
		if (editPart instanceof ClassOperations2EditPart) {
	...
	public List/*<org.eclipse.gmf.runtime.emf.type.core.IElementType>*/getMARelTypesOnSourceAndTarget(IGraphicalEditPart targetEditPart) {
		...
		if (targetEditPart instanceof org.eclipse.uml2.diagram.clazz.edit.parts.Class5EditPart) {
			types.add(UMLElementTypes.Generalization_4001);
		}
	...
	public EditPart getTargetEditPart(Request request) {
		if (request instanceof CreateViewAndElementRequest) {
			...
			if (type == UMLElementTypes.Property_3001) {
				return getChildBySemanticHint(UMLVisualIDRegistry.getType(ClassAttributes2EditPart.VISUAL_ID));
			}
			if (type == UMLElementTypes.Operation_3002) {
				return getChildBySemanticHint(UMLVisualIDRegistry.getType(ClassOperations2EditPart.VISUAL_ID));
			}
			if (type == UMLElementTypes.Class_3003) {
				return getChildBySemanticHint(UMLVisualIDRegistry.getType(ClassClasses2EditPart.VISUAL_ID));
			}
		}
	...
	protected void addSemanticListeners() {
		...
		for (IUpdaterNodeDescriptor next : getClass_3033ContainedLinks()) {
	...
	private List<IUpdaterLinkDescriptor> getClass_3033ContainedLinks() {
		return UMLDiagramUpdater.getClass_3033ContainedLinks(getNotationView());
	}
	...
	protected EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(ClassStereotype2EditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
