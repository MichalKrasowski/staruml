...
import org.eclipse.uml2.diagram.clazz.edit.policies.DataType2CanonicalEditPolicy;
 ...
 * @generated
 */
public class DataType2EditPart extends AbstractBorderedShapeEditPart implements PrimaryShapeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof DataTypeNameEditPart) {
			((DataTypeNameEditPart) childEditPart).setLabel(getPrimaryShape().getFigureClassFigure_NameLabel());
			return true;
		}
		if (childEditPart instanceof DataTypeStereotypeEditPart) {
			((DataTypeStereotypeEditPart) childEditPart).setLabel(getPrimaryShape().getFigureClassFigure_StereoLabel());
			return true;
		}
	...
	public List/*<org.eclipse.gmf.runtime.emf.type.core.IElementType>*/getMARelTypesOnSourceAndTarget(IGraphicalEditPart targetEditPart) {
		...
		if (targetEditPart instanceof org.eclipse.uml2.diagram.clazz.edit.parts.DataType2EditPart) {
			types.add(UMLElementTypes.Generalization_4001);
		}
	...
	public EditPart getTargetEditPart(Request request) {
		if (request instanceof CreateViewAndElementRequest) {
			...
			if (type == UMLElementTypes.Property_3014) {
				return getChildBySemanticHint(UMLVisualIDRegistry.getType(DataTypeAttributesEditPart.VISUAL_ID));
			}
			if (type == UMLElementTypes.Operation_3015) {
				return getChildBySemanticHint(UMLVisualIDRegistry.getType(DataTypeOperationsEditPart.VISUAL_ID));
			}
		}
	...
	protected void addSemanticListeners() {
		...
		for (IUpdaterNodeDescriptor next : getDataType_2004ContainedLinks()) {
	...
	private List<IUpdaterLinkDescriptor> getDataType_2004ContainedLinks() {
		return UMLDiagramUpdater.getDataType_2004ContainedLinks(getNotationView());
	}
	...
	protected EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(DataTypeStereotypeEditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
