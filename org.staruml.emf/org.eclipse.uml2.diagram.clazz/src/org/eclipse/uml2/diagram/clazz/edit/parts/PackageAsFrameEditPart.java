...
public class PackageAsFrameEditPart extends ShapeNodeEditPart implements PrimaryShapeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof PackageName3EditPart) {
			((PackageName3EditPart) childEditPart).setLabel(getPrimaryShape().getPackageAsFrameFigure_name());
			return true;
		}
		if (childEditPart instanceof PackageAsFrameContentsEditPart) {
			IFigure pane = getPrimaryShape().getPackageAsFrameFigure_contents();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.add(((PackageAsFrameContentsEditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof PackageAsFrameContentsEditPart) {
			IFigure pane = getPrimaryShape().getPackageAsFrameFigure_contents();
			pane.remove(((PackageAsFrameContentsEditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	public List/*<org.eclipse.gmf.runtime.emf.type.core.IElementType>*/getMARelTypesOnSourceAndTarget(IGraphicalEditPart targetEditPart) {
		...
		if (targetEditPart instanceof org.eclipse.uml2.diagram.clazz.edit.parts.PackageAsFrameEditPart) {
			types.add(UMLElementTypes.Dependency_4002);
		}
	...
	protected void addSemanticListeners() {
		...
		for (IUpdaterNodeDescriptor next : getPackage_2016ContainedLinks()) {
