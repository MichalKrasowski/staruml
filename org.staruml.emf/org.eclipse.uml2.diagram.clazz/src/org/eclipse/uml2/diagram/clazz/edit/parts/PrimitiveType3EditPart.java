...
public class PrimitiveType3EditPart extends AbstractBorderedShapeEditPart implements PrimaryShapeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof PrimitiveTypeName2EditPart) {
			((PrimitiveTypeName2EditPart) childEditPart).setLabel(getPrimaryShape().getFigureClassFigure_NameLabel());
			return true;
		}
		if (childEditPart instanceof PrimitiveTypeStereotype2EditPart) {
			((PrimitiveTypeStereotype2EditPart) childEditPart).setLabel(getPrimaryShape().getFigureClassFigure_StereoLabel());
			return true;
		}
	...
	public List/*<org.eclipse.gmf.runtime.emf.type.core.IElementType>*/getMARelTypesOnSourceAndTarget(IGraphicalEditPart targetEditPart) {
		...
		if (targetEditPart instanceof org.eclipse.uml2.diagram.clazz.edit.parts.PrimitiveType3EditPart) {
			types.add(UMLElementTypes.Generalization_4001);
		}
	...
	public EditPart getTargetEditPart(Request request) {
		if (request instanceof CreateViewAndElementRequest) {
			...
			if (type == UMLElementTypes.Property_3021) {
				return getChildBySemanticHint(UMLVisualIDRegistry.getType(PrimitiveTypeAttributes2EditPart.VISUAL_ID));
			}
			if (type == UMLElementTypes.Operation_3022) {
				return getChildBySemanticHint(UMLVisualIDRegistry.getType(PrimitiveTypeOperations2EditPart.VISUAL_ID));
			}
		}
	...
	protected EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(PrimitiveTypeStereotype2EditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
