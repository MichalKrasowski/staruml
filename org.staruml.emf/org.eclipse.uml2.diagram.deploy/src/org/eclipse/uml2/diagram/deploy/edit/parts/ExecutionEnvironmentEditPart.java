...
public class ExecutionEnvironmentEditPart extends ShapeNodeEditPart implements PrimaryShapeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof ExecutionEnvironmentStereoEditPart) {
			((ExecutionEnvironmentStereoEditPart) childEditPart).setLabel(getPrimaryShape().getEnvironmentTypeLabel());
			return true;
		}
	...
	public EditPart getTargetEditPart(Request request) {
		if (request instanceof CreateViewAndElementRequest) {
			...
			if (type == UMLElementTypes.Artifact_3006) {
				return getChildBySemanticHint(UMLVisualIDRegistry.getType(ExecutionEnvironmentArtifactsEditPart.VISUAL_ID));
			}
		}
