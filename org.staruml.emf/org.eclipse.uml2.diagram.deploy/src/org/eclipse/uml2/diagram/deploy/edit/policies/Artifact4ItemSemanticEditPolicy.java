...
public class Artifact4ItemSemanticEditPolicy extends UMLBaseItemSemanticEditPolicy {

	/**
	 * @generated
	 */
	public Artifact4ItemSemanticEditPolicy() {
		super(UMLElementTypes.Artifact_3008);
	}
				...
				for (Iterator cit = node.getChildren().iterator(); cit.hasNext();) {
					...
					case DeploymentSpecification2EditPart.VISUAL_ID:
						for (Iterator it = cnode.getTargetEdges().iterator(); it.hasNext();) {
							...
							if (UMLVisualIDRegistry.getVisualID(incomingLink) == DeploymentConfigurationEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(incomingLink.getSource().getElement(), null, incomingLink.getTarget().getElement(), false);
								cmd.add(new DestroyReferenceCommand(r) {
