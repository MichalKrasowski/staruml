...
public class UMLNewDiagramFileWizard extends Wizard {
	...
	private void createSynchronizationPage(TransactionalEditingDomain editingDomain) {
		synchronizationPage = new UMLProfileSynchronizationWizardPage("Select diagram synchronization scheme", editingDomain);
	...
	private void createFileCreationPage(URI domainModelURI) {
		...
		myFileCreationPage.setDescription(NLS.bind(Messages.UMLNewDiagramFileWizard_CreationPageDescription, ProfileEditPart.MODEL_ID));
		IPath filePath;
		...
		myFileCreationPage.setFileName(UMLDiagramEditorUtil.getUniqueFileName(filePath, fileName, "umlprofile")); //$NON-NLS-1$
	}
	...
	private AbstractTransactionalCommand getCreateDiagramCommand(final Resource diagramResource, List affectedFiles) {
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(myEditingDomain, Messages.UMLNewDiagramFileWizard_InitDiagramCommand, affectedFiles) {
			...
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info) throws ExecutionException {
				...
				if (diagram == null) {
					diagram = ViewService.createDiagram(diagramRootElementSelectionPage.getModelElement(), ProfileEditPart.MODEL_ID, UMLDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT);
				}
