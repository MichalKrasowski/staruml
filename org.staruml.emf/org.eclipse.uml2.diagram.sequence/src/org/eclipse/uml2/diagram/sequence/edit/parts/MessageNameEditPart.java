...
public class MessageNameEditPart extends LabelEditPart implements ITextAwareEditPart {
	 ...
	 */
	static {
		registerSnapBackPosition(UMLVisualIDRegistry.getType(org.eclipse.uml2.diagram.sequence.edit.parts.MessageNameEditPart.VISUAL_ID), new Point(0, -10));
	}
	...
	protected EObject getParserElement() {
		return getNotationView(); //MessageNumberParser needs view in order to obtain shared SDModel instance
	}
	...
	protected String getLabelText() {
		...
		if (parserElement != null && getParser() != null) {
			text = getParser().getPrintString(new EObjectAdapter(getNotationView()), getParserOptions().intValue());
		}
	...
	public IParser getParser() {
		if (parser == null) {
			parser = UMLParserProvider.getParser(UMLElementTypes.Message_4001, resolveSemanticElement(), UMLVisualIDRegistry
					.getType(org.eclipse.uml2.diagram.sequence.edit.parts.MessageNameEditPart.VISUAL_ID));
		}
		return parser;
	}
	...
	protected void addSemanticListeners() {
		if (getParser() instanceof ISemanticParser) {
			...
			parserElements = ((ISemanticParser) getParser()).getSemanticElementsBeingParsed(getNotationView()); //SIC!
	...
	protected IFigure createFigurePrim() {
		return new Message_label();
	}

	/**
	 * @generated
	 */
	public class Message_label extends Label {
	...
	private boolean isVisibleByPreferences() {
		return DiagramIconStylePreferenceHelper.shouldShowLabel(MessageNameEditPart.VISUAL_ID, getDiagramPreferencesHint());
	}
