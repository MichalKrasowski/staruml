...
import org.eclipse.uml2.diagram.activity.edit.policies.StructuredActivityNodeCanonicalEditPolicy;
...
public class StructuredActivityNodeEditPart extends ShapeNodeEditPart implements PrimaryShapeEditPart {
	...
	protected void createDefaultEditPolicies() {
		...
		installEditPolicy(EditPolicyRoles.CANONICAL_ROLE, new StructuredActivityNodeCanonicalEditPolicy());
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof StructuredActivityNodeNameEditPart) {
			((StructuredActivityNodeNameEditPart) childEditPart).setLabel(getPrimaryShape().getFigureStructuredActivityFigure_name());
			return true;
		}
		if (childEditPart instanceof StructuredActivityNodeStereotypeEditPart) {
	...
	public EditPart getPrimaryChildEditPart() {
		return getChildBySemanticHint(UMLVisualIDRegistry.getType(StructuredActivityNodeNameEditPart.VISUAL_ID));
	}
	...
	protected void addSemanticListeners() {
		...
		for (IUpdaterNodeDescriptor next : getStructuredActivityNode_3046ContainedLinks()) {
	...
	private List<IUpdaterLinkDescriptor> getStructuredActivityNode_3046ContainedLinks() {
		return UMLDiagramUpdater.getStructuredActivityNode_3046ContainedLinks(getNotationView());
	}
	...
	protected EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(StructuredActivityNodeStereotypeEditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
