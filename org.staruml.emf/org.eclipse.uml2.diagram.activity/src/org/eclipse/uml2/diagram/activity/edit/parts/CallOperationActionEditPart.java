...
public class CallOperationActionEditPart extends AbstractBorderedShapeEditPart implements PrimaryShapeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof CallOperationActionStereotypeEditPart) {
	...
	protected void addSemanticListeners() {
		...
		for (IUpdaterNodeDescriptor next : getCallOperationAction_3045ContainedLinks()) {
	...
	private List<IUpdaterLinkDescriptor> getCallOperationAction_3045ContainedLinks() {
		return UMLDiagramUpdater.getCallOperationAction_3045ContainedLinks(getNotationView());
	}
	...
	protected EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(CallOperationActionStereotypeEditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
