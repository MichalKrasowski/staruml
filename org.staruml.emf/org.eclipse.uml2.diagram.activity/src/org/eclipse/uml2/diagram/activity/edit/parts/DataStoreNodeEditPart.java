...
public class DataStoreNodeEditPart extends AbstractBorderedShapeEditPart implements PrimaryShapeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof DataStoreNodeInStateEditPart) {
			((DataStoreNodeInStateEditPart) childEditPart).setLabel(getPrimaryShape().getFigureDataStoreFigure_states());
			return true;
		}
		if (childEditPart instanceof DataStoreNodeStereotypeEditPart) {
			((DataStoreNodeStereotypeEditPart) childEditPart).setLabel(getPrimaryShape().getFigureDataStoreFigure_stereo());
			return true;
		}
		return false;
	}
	...
	protected void addBorderItem(IFigure borderItemContainer, IBorderItemEditPart borderItemEditPart) {
		if (borderItemEditPart instanceof DataStoreNodeOrderingEditPart) {
	...
	public List/*<org.eclipse.gmf.runtime.emf.type.core.IElementType>*/getMARelTypesOnSourceAndTarget(IGraphicalEditPart targetEditPart) {
		...
		if (targetEditPart instanceof org.eclipse.uml2.diagram.activity.edit.parts.DataStoreNodeEditPart) {
			types.add(UMLElementTypes.ControlFlow_4001);
		}
	...
	protected EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(DataStoreNodeStereotypeEditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
