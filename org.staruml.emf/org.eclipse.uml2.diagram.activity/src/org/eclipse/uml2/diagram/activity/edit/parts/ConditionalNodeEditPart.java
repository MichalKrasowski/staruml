...
public class ConditionalNodeEditPart extends ShapeNodeEditPart implements PrimaryShapeEditPart {
	...
	protected void createDefaultEditPolicies() {
		...
		installEditPolicy(EditPolicyRoles.CANONICAL_ROLE, new ConditionalNodeCanonicalEditPolicy());
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof ConditionalNodeCompartmentEditPart) {
			...
			pane.add(((ConditionalNodeCompartmentEditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof ConditionalNodeCompartmentEditPart) {
			IFigure pane = getPrimaryShape().getFigureStructuredActivityFigure_ContentPane();
			pane.remove(((ConditionalNodeCompartmentEditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	public List/*<org.eclipse.gmf.runtime.emf.type.core.IElementType>*/getMARelTypesOnSourceAndTarget(IGraphicalEditPart targetEditPart) {
		...
		if (targetEditPart instanceof org.eclipse.uml2.diagram.activity.edit.parts.ConditionalNodeEditPart) {
			types.add(UMLElementTypes.ControlFlow_4001);
		}
	...
	protected void addSemanticListeners() {
		...
		for (IUpdaterNodeDescriptor next : getConditionalNode_3082ContainedLinks()) {
	...
	private List<IUpdaterLinkDescriptor> getConditionalNode_3082ContainedLinks() {
		return UMLDiagramUpdater.getConditionalNode_3082ContainedLinks(getNotationView());
	}
	...
	protected EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(ConditionalNodeStereotypeEditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
