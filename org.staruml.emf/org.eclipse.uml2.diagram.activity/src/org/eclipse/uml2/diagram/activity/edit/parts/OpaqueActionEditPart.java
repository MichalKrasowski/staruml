...
public class OpaqueActionEditPart extends AbstractBorderedShapeEditPart implements PrimaryShapeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof OpaqueActionStereotypeEditPart) {
			((OpaqueActionStereotypeEditPart) childEditPart).setLabel(getPrimaryShape().getFigureActionBaseFigure_stereo());
			return true;
		}
	...
	public List/*<org.eclipse.gmf.runtime.emf.type.core.IElementType>*/getMARelTypesOnSourceAndTarget(IGraphicalEditPart targetEditPart) {
		...
		if (targetEditPart instanceof org.eclipse.uml2.diagram.activity.edit.parts.OpaqueActionEditPart) {
			types.add(UMLElementTypes.ControlFlow_4001);
		}
	...
	protected void addSemanticListeners() {
		...
		for (IUpdaterNodeDescriptor next : getOpaqueAction_3029ContainedLinks()) {
	...
	private List<IUpdaterLinkDescriptor> getOpaqueAction_3029ContainedLinks() {
		return UMLDiagramUpdater.getOpaqueAction_3029ContainedLinks(getNotationView());
	}
	...
	protected EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(OpaqueActionStereotypeEditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
