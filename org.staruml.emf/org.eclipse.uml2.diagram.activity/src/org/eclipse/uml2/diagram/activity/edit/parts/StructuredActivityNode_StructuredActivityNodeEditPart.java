...
public class StructuredActivityNode_StructuredActivityNodeEditPart extends ShapeNodeEditPart implements PrimaryShapeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof StructuredActivityNode_StructuredActivityNodeContentPaneCompartmentEditPart) {
			IFigure pane = getPrimaryShape().getFigureStructuredActivityFigure_ContentPane();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.add(((StructuredActivityNode_StructuredActivityNodeContentPaneCompartmentEditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof StructuredActivityNode_StructuredActivityNodeContentPaneCompartmentEditPart) {
			IFigure pane = getPrimaryShape().getFigureStructuredActivityFigure_ContentPane();
			pane.remove(((StructuredActivityNode_StructuredActivityNodeContentPaneCompartmentEditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	protected EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(StructuredActivityNode_StructuredActivityNodeStereotypeEditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
