...
public class ActivityPartition_ConditionalNodeEditPart extends ShapeNodeEditPart implements PrimaryShapeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof ActivityPartition_ConditionalNodeNameEditPart) {
			((ActivityPartition_ConditionalNodeNameEditPart) childEditPart).setLabel(getPrimaryShape().getFigureStructuredActivityFigure_name());
			return true;
		}
		...
		if (childEditPart instanceof ActivityPartition_ConditionalNodeCompartmentEditPart) {
			IFigure pane = getPrimaryShape().getFigureStructuredActivityFigure_ContentPane();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.add(((ActivityPartition_ConditionalNodeCompartmentEditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof ActivityPartition_ConditionalNodeCompartmentEditPart) {
			IFigure pane = getPrimaryShape().getFigureStructuredActivityFigure_ContentPane();
			pane.remove(((ActivityPartition_ConditionalNodeCompartmentEditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	public EditPart getPrimaryChildEditPart() {
		return getChildBySemanticHint(UMLVisualIDRegistry.getType(ActivityPartition_ConditionalNodeNameEditPart.VISUAL_ID));
	}
	...
	protected EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(ActivityPartition_ConditionalNodeStereotypeEditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
