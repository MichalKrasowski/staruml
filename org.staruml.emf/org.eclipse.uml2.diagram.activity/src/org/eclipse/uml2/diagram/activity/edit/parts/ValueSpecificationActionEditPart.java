...
import org.eclipse.uml2.diagram.activity.edit.policies.ValueSpecificationActionCanonicalEditPolicy;
import org.eclipse.uml2.diagram.activity.edit.policies.ValueSpecificationActionItemSemanticEditPolicy;
...
public class ValueSpecificationActionEditPart extends AbstractBorderedShapeEditPart implements PrimaryShapeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof ValueSpecificationActionNameEditPart) {
			((ValueSpecificationActionNameEditPart) childEditPart).setLabel(getPrimaryShape().getFigureActionBaseFigure_name());
			return true;
		}
		if (childEditPart instanceof ValueSpecificationActionStereotypeEditPart) {
	...
	protected EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(ValueSpecificationActionStereotypeEditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
