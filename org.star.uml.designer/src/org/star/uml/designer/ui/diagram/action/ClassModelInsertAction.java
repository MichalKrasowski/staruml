...
public class ClassModelInsertAction extends Action implements IStarUMLModelAction{
	...
	public void run() {
		...
		try{
			...
			// 모델에 이미 Class가 추가되어 있기 때문에 모델과 Snyc가능한 Class중 원하는 것을 선택 한 후 추가한다.
			...
			if(page.getActiveEditor() !=null && page.getActiveEditor() instanceof org.eclipse.uml2.diagram.clazz.part.UMLDiagramEditor){
	        	...
	        	if(visilityFlag){
		        	...
		        	Diagram syncDiagram = ViewService.createDiagram(document.getDiagram().getElement(),"UMLClass", ep.getDiagramPreferencesHint());
	        	...
	        	for(int i=0; i<diagram.getPersistedChildren().size(); i++){
	        		...
	        		if(shapeImple.getElement() instanceof ClassImpl){
	        			...
	        			ClassImpl impl = (ClassImpl)shapeImple.getElement();
